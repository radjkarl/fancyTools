...
def vignettingEq((x, y), f=100, alpha=0, Xi=0, tau=0,cx=50,cy=50):
    '''
    ...
    A = 1.0/(1+(dist/f)**2)**2
    G = (1-alpha*dist)
    T = np.cos(tau) * (1+(np.tan(tau)/f) * (x*np.sin(Xi)-y*np.cos(Xi)) )**3
...
def fitVignettingEq(arr, mask, scale_factor=0.1):
    ...
    z = small[mask]
    #INITIAL GUESS:
    guess = (small.shape[0]*0.7, 0, 0, 0,small.shape[0]/2.0,small.shape[1]/2.0)
    #FIT:
    ...
    rebuilt = np.fromfunction(lambda x,y: vignettingEq((y*fy,x*fx),*parameters), arr.shape)
    return rebuilt, perr

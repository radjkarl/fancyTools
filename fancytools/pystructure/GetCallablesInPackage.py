        ...
        for (name, obj) in inspect.getmembers(module):
            ...
            if inspect.ismodule(obj):
                ...
                if obj.__name__.startswith(self._package.__name__):
                    # either use module for submenus or check whether module is
                    # actually a package:
                    if self._modules_in_structure or obj.__file__.endswith(
                            '__init__.pyc'):
                        ...
                        # use same menu for all classes of all files on one
                        # folder/module
                ...
                if (
                    # ( not self._include_classes or ( self._include_classes and inspect.isclass(obj) ) or
                    (self._include_classes and inspect.isclass(obj)) or
                    # and/or a function
                   # ( not self._include_functions or ( self._include_functions and inspect.isfunction(obj)) )
                   (self._include_functions and inspect.isfunction(obj))
                   ):
                        # if object belongs to its parent module (mod is not imported)
                        # OR module paths are the same (in case of from mod
                        # import cls and cls.name == mod.name)
                    try:
